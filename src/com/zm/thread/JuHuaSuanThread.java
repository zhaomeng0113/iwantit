			...
			if(json.get("floorName").toString().equals("热卖爆品")||json.get("floorName").toString().equals("热门爆款")||json.get("floorName").toString().equals("热门商品")) {
				...
				if(!StringUtils.isEmpty(list)){
					for (Object object : list) {
						...
						//判断是否免单
						if(currentGoodsName.contains("京润珍珠")||currentGoodsName.contains("碧欧泉")||currentGoodsName.contains("兰芝")||currentGoodsName.contains("小虫米子")||currentGoodsName.contains("百雀羚")||currentGoodsName.contains("骆驼")||currentGoodsName.contains("迪奥")||currentGoodsName.contains("天喔")){
							if(!flowAct.getEmpty()){
								Object mcVal = mc.get(flowAct.getFlowId()+"");
								logger.info("memcached获取值："+mcVal);
								if(!currentGoodsName.equals(mcVal)){
